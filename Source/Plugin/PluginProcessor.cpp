...
EarlyPocketAudioProcessor::EarlyPocketAudioProcessor()
 ...
 const char* ids[]={"roomSize","faces","roomShape","count","width","distance","mix","hp","eq1Freq","eq1Gain","eq2Freq","eq2Gain","eq3Freq","eq3Gain","lp","bypass"};for(size_t i=0;i<raw.size();++i)raw[i]=parameters.getRawParameterValue(ids[i]);startTimerHz(20);
}
...
juce::AudioProcessorValueTreeState::ParameterLayout EarlyPocketAudioProcessor::layout(){
 ...
 p.push_back(std::make_unique<juce::AudioParameterInt>("faces","Faces",1,16,4));
 p.push_back(std::make_unique<juce::AudioParameterFloat>("roomShape","Room Shape",0.f,100.f,20.f));
 p.push_back(std::make_unique<juce::AudioParameterInt>("count","Count",1,24,6));
...
void EarlyPocketAudioProcessor::updateModelAndEq(){
    const std::array<float,6> modelValues{raw[0]->load(),raw[2]->load(),raw[3]->load(),raw[4]->load(),raw[5]->load(),raw[1]->load()};
    if(!modelCacheValid||modelValues!=lastModelValues){
        early::Parameters p{modelValues[0]*.01f,modelValues[1]*.01f,int(std::lround(modelValues[2])),modelValues[3]*.01f,modelValues[4]*.01f,int(std::lround(modelValues[5]))};
        engine.setModel(early::buildModel(p));
        lastModelValues=modelValues;modelCacheValid=true;
    }
    const std::array<float,8> eqValues{raw[7]->load(),raw[8]->load(),raw[9]->load(),raw[10]->load(),raw[11]->load(),raw[12]->load(),raw[13]->load(),raw[14]->load()};
...
void EarlyPocketAudioProcessor::applyFit(){early::FitResult fit;early::TargetSummary t;{const juce::SpinLock::ScopedLockType lock(dataLock);fit=pendingFit;t=pendingTarget;}if(!fit.valid){learnState=LearnState::insufficient;return;}const std::array<const char*,9> ids{"roomSize","roomShape","count","width","distance","eq1Gain","eq2Gain","eq3Gain","faces"};const std::array<float,9> values{fit.parameters.roomSize*100,fit.parameters.roomShape*100,float(fit.parameters.count),fit.parameters.width*100,fit.parameters.pattern*100,fit.eqGainDb[0],fit.eqGainDb[1],fit.eqGainDb[2],float(fit.parameters.faces)};for(size_t i=0;i<ids.size();++i)if(auto* p=parameters.getParameter(ids[i])){p->beginChangeGesture();p->setValueNotifyingHost(p->convertTo0to1(values[i]));p->endChangeGesture();}{const juce::SpinLock::ScopedLockType lock(dataLock);target=t;}doneAtMs=juce::Time::getMillisecondCounterHiRes();learnState=LearnState::ready;}

early::TapModel EarlyPocketAudioProcessor::getDisplayedModel()const{early::Parameters p{raw[0]->load()*.01f,raw[2]->load()*.01f,int(std::lround(raw[3]->load())),raw[4]->load()*.01f,raw[5]->load()*.01f,int(std::lround(raw[1]->load()))};return early::buildModel(p);}
