...
namespace {
constexpr float designWidth=1672.f, expandedHeight=1065.f, collapsedHeight=741.f;
...
void drawDialBody(juce::Graphics& g,juce::Rectangle<float> area,PocketLook& look,const juce::String& title,const juce::String& subtitle,const juce::String& unit,float value,float proportion,bool integer){
    ...
    const juce::String shown=integer?juce::String(juce::roundToInt(value)):juce::String(juce::roundToInt(value))+unit;text(g,title,{c.x-r,c.y-46.f*s,2*r,25.f*s},18.f*s,look.ink(),juce::Justification::centred);text(g,shown,{c.x-r,c.y-19.f*s,2*r,43.f*s},(unit=="%"?33.f:30.f)*s,look.ink(),juce::Justification::centred);text(g,subtitle,{c.x-r,c.y+28.f*s,2*r,23.f*s},13.f*s,look.muted(),juce::Justification::centred);
}
...
ModernDial::ModernDial(PocketLook& l,juce::String t,juce::String sub,juce::String u,bool isInteger):look(l),title(std::move(t)),subtitle(std::move(sub)),unit(std::move(u)),integer(isInteger){setSliderStyle(juce::Slider::RotaryVerticalDrag);setTextBoxStyle(juce::Slider::NoTextBox,false,0,0);setName(title);setWantsKeyboardFocus(true);}
void ModernDial::paint(juce::Graphics& g){
    drawDialBody(g,getLocalBounds().toFloat(),look,title,subtitle,unit,float(getValue()),float(valueToProportionOfLength(getValue())),integer);
}
...
void EarlyPocketAudioProcessorEditor::resized(){graph.setBounds(scaled(39,98,1572,330));eq.setBounds(scaled(39,741,1572,300));eq.setVisible(expanded);blurArea=scaled(24,84,1624,expanded?expandedHeight-84:collapsedHeight-84);plugin.editorWidth.store(getWidth());}
