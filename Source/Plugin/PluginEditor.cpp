...
void EqualizerGraph::paint(juce::Graphics& g){
    ...
    juce::Path curve;for(int x=0;x<=int(p.getWidth());++x){const float f=20.f*std::pow(1000.f,float(x)/p.getWidth());const float db=exactEqResponseDb(sr,f,hp,f1,a1,f2,a2,q,f3,a3,lp);const float y=yForDb(db);if(x==0)curve.startNewSubPath(p.getX(),y);else curve.lineTo(p.getX()+float(x),y);}juce::Path area=curve;area.lineTo(p.getRight(),yForDb(0));area.lineTo(p.getX(),yForDb(0));area.closeSubPath();g.setColour(juce::Colour(0x148f8f8f));g.fillPath(area);g.setColour(juce::Colour(0xffe8e8e8));g.strokePath(curve,juce::PathStrokeType(juce::jmax(1.2f,1.8f*ui)));
